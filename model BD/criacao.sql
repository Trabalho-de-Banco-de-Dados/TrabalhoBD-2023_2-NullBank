...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`Agencia` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`Funcionario` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`Dependente` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`Endereco` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`Cliente` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`ContaBancaria` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`Telefone` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`Transacao` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`Email` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`contaCorrente` ;

CREATE TABLE IF NOT EXISTS `nullbank`.`contaCorrente` (
  `aniversario_contrato` CHAR(50) NOT NULL,
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`ContaPoupanca` ;
...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `nullbank`.`ContaEspecial` ;
...
USE `nullbank`$$
DROP TRIGGER IF EXISTS `nullbank`.`SalarioBaixoInsert` $$
USE `nullbank`$$
...
USE `nullbank`$$
DROP TRIGGER IF EXISTS `nullbank`.`SomaSalarioTotalInsert` $$
USE `nullbank`$$
...
USE `nullbank`$$
DROP TRIGGER IF EXISTS `nullbank`.`SalarioBaixoUpdate` $$
USE `nullbank`$$
...
USE `nullbank`$$
DROP TRIGGER IF EXISTS `nullbank`.`SomaSalarioTotalUpdate` $$
USE `nullbank`$$
...
USE `nullbank`$$
DROP TRIGGER IF EXISTS `nullbank`.`SomaSalarioTotalDelete` $$
USE `nullbank`$$
...
BEGIN
    ...
    -- Calcular o total de salários para a agência
    SELECT COALESCE(SUM(salario), 0) INTO total_salario
    FROM Funcionario
...
USE `nullbank`$$
DROP TRIGGER IF EXISTS `nullbank`.`NumeroDependentesInsert` $$
USE `nullbank`$$
...
USE `nullbank`$$
DROP TRIGGER IF EXISTS `nullbank`.`NumeroDependentesDelete` $$
USE `nullbank`$$
...
USE `nullbank`$$
DROP TRIGGER IF EXISTS `nullbank`.`ContaBancaria_BEFORE_INSERT` $$
USE `nullbank`$$
CREATE DEFINER = CURRENT_USER TRIGGER `nullbank`.`ContaBancaria_BEFORE_INSERT` BEFORE INSERT ON `ContaBancaria` FOR EACH ROW
BEGIN
	DECLARE num_contas INT;
    -- Verificar o número de contas do cliente na agência
    SELECT COUNT(*) INTO num_contas
    FROM ContaBancaria
    WHERE Cliente_cpf = NEW.Cliente_cpf AND Agencia_idAgencia = NEW.Agencia_idAgencia;

    -- Se o cliente já tiver uma conta na agência, lançar um erro
...
USE `nullbank`$$
DROP TRIGGER IF EXISTS `nullbank`.`Transacao_AFTER_INSERT` $$
USE `nullbank`$$
...
BEGIN
    ...
    IF NEW.tipo_transacao = 'deposito' OR NEW.tipo_transacao = 'estorno' OR NEW.tipo_transacao = 'pagamento' THEN
    ...
    ELSE
		IF NEW.tipo_transacao = 'saque' THEN
			SET novo_saldo = (SELECT saldo - NEW.valor FROM ContaBancaria WHERE numero_conta = NEW.Saida);
			...
			UPDATE ContaBancaria SET saldo = novo_saldo WHERE numero_conta = NEW.Saida;
		ELSE
			SET novo_saldo = (SELECT saldo - NEW.valor FROM ContaBancaria WHERE numero_conta = NEW.Saida);
